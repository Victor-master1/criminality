  ...
  return (
    <div className="space-y-8 animate-slide-up">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-2">Datasets</h1>
          <p className="text-slate-600 text-lg">Administra tus conjuntos de datos</p>
        </div>
        <label className="btn-primary cursor-pointer space-x-2">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
          </svg>
          <span>{subiendo ? 'Subiendo...' : 'Subir Dataset'}</span>
          <input
            type="file"
            accept=".csv"
      ...
      {datasets.length === 0 ? (
        <div className="card p-16 text-center">
          ...
          <h3 className="text-2xl font-bold text-slate-900 mb-2">No hay datasets</h3>
          <p className="text-slate-600 text-lg mb-6">Sube tu primer dataset CSV para comenzar</p>
          ...
          {datasets.map((dataset, index) => (
            ...
            >
              ...
              <div
                ...
                onClick={() => navigate(`/limpieza/${dataset.id}`)}
              >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl animate-scale-in">
            ...
            <p className="text-slate-600 text-center mb-6">
              Se eliminará <span className="font-semibold text-slate-900">{modalEliminar.nombre}</span> y todos los experimentos asociados. Esta acción no se puede deshacer.
            </p>
